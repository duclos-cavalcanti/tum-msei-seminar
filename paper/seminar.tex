...
techniques such as evolutionary algorithms (EA), Swarm Intelligence (SI) methods and many others. Since the vast majority
of relevant ENAS research has applied some form of EA, this will be the major focus of this work.
...
small effort as microcontrollers posess very limited resources, especially concerning memory and storage.

\cite{lin2020mcunet} shows promising results in that regard with the development of the MCUNet framework.
MCUNet includes two major components, the efficient neural architecture (TinyNAS) and the lightweight inference engine (TinyEngine).
...
through extra constaints shrinks the search space such that it only considers architectures that fit the desired requirements,
which can be memory consumption, storage, latency and even energy.
...
respectively \cite{lin2020mcunet}. MCUnet achieved state of the art performance, taking 300 GPU hours to design a model
\cite{lin2020mcunet}. This is a great improvement compared to MnasNET, which took 40,000 GPU hours for the same
data set \cite{tan2019mnasnet}.
