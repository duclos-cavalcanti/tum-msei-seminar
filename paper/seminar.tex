...
\begin{itemize}
    \item \textbf{Search Space}: Space, which defines all the possible architectures that can in principle be considered
    by the algorithm.
...
explore unseen architectures and also possibly restricting the complexity of the design.
...
of NAS and gradient-based optimization, frequently sub-optimal architectures are found \cite{liu2021survey}. This also
creates an
...
gradient information \cite{liu2021survey}. NAS algorithms that leverage the use of EC techniques are also called
\textit{Evolutionary Neural Architectecture Search} (ENAS) algorithms.
